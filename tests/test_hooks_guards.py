...
class TestPreEditHumanOnly:
    ...
    @pytest.mark.parametrize(
        ...
        [
            "{m}",  # bare
            "  {m}",  # leading whitespace
            "# {m}",  # shell/python
            "// {m}",  # c/js
            "/* {m} */",  # c block
            " * {m}",  # jsdoc continuation
            "; {m}",  # ini/lisp/asm
            "-- {m}",  # sql/lua/haskell
            "<!-- {m} -->",  # html/markdown
            "<!--{m}",  # unterminated html — fail-safe deny (marker-led)
            "<!--{m}-->",  # html, NO spaces around marker (common form)
            "/*{m}*/",  # c block, NO spaces (common form)
            "{m}-->",  # marker glued to closer, no opener
            "# {m} do not edit this file",  # marker + trailing note
        ],
    )
    def test_marker_led_lines_deny(self, hooks_dir, tmp_path, line):
        """Marker-led lines across comment styles deny (fail-safe protect)."""
        repo = self._make_repo(tmp_path)
        marker = "HUMAN" + "-ONLY"
        target = repo / "marked.txt"
        target.write_text("first line\n" + line.format(m=marker) + "\nlast line\n")
        r = _run_hook(
            self._hook(hooks_dir),
            {"tool_input": {"file_path": str(target)}},
            cwd=repo,
    ...
    @pytest.mark.parametrize(
        "line",
        [
            'Block edits to {m} marked files',  # buried mid-sentence (the hooks.json bug)
            "# this file is {m} protected",  # marker buried after words in a comment
            "| {m} | desc |",  # markdown table cell (asset-charter.md style)
            "see {m}-policy.md for details",  # marker glued into a token
        ],
    )
    def test_inline_marker_mentions_pass(self, hooks_dir, tmp_path, line):
        """A marker buried in prose / a table / a token is NOT a self-marker."""
        repo = self._make_repo(tmp_path)
        marker = "HUMAN" + "-ONLY"
        target = repo / "prose.md"
