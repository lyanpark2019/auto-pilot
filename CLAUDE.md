...
- `skills/` — 7 dirs / 7 active: setup-harness (+ its `scripts/`, `references/`, `templates/`, `evals/`; loop scoring delegates to ARL codebase mode), adversarial-review-loop (branch/codebase/multi-agent + `--lifecycle` mode — absorbed pm-quality-harness-loop 2026-06-07), quality-eval (rubric SoT, data not entry point), residue-audit, sha-deploy-standard, doc-management (the docs subsystem — REBUILD/MAINTAIN/AUDIT modes; absorbs the retired graphify-doc-rebuild / doc-drift-audit / doc-sync / llm-wiki-architect skills)
...
| `scripts/_identity.py` | slim | neutral identity/path utils re-homed from the removed learning loop: `PLUGIN_VERSION`, `project_slug`, `repo_fingerprint`, `ledger_lock`, `resolve_vault` — consumed by escalation + enrichment |
...
| `scripts/_escalation_emit.py` | inc3-escalation | best-effort tier-1->tier-2 boundary emit; wraps _escalation.bump_or_create at the tier-1 give-up points, never raises (additive) |
...
python3 hooks/test_guard_destructive.py && python3 hooks/test_notebooklm_delete_gate.py && python3 hooks/test_dispatch_contract_gate.py && python3 hooks/test_headless_sync_dispatch_guard.py && python3 hooks/test_verifier_tier_gate.py && python3 hooks/test_pre_edit_human_only.py && python3 hooks/test_preflight_handoff.py && python3 hooks/test_subagent_deliverable_check.py && python3 hooks/test_doc_sync_update.py && python3 hooks/test_session_distill_stop.py  # bundled hook self-tests (script-style, not pytest)
...
- **Codex review = `/codex:adversarial-review` slash (DEFAULT); avoid direct `codex exec`** — for any "codex review" / "적대적 리뷰" / code-review request, invoke the `/codex:adversarial-review` slash. Direct `codex exec` is prone to **hangs and slowness** in this environment → do NOT hand-roll it for review. Reach for raw `codex exec -s read-only` ONLY inside `auto-pilot:adversarial-review-loop` codebase/multi-agent fan-out, or a quick working-tree partial-file pass where the slash's PR/branch framing doesn't fit (say so when you do).
- **Non-interactive Codex/CLI subagents must consume stdin** — when raw `codex exec` IS unavoidable (the two exceptions above), feed a prompt (`--prompt-file - < file` / heredoc / inline arg) or redirect `< /dev/null`. A bare `codex exec` inherits the TTY and hangs. (auto-pilot's reviewers already do this; applies to any new codex call.)
