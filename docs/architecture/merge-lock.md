...
Design proposal for autopilot-swarm M2 race-hardening. Target file:
`/Users/lyan/.claude/plugins/autopilot-swarm/scripts/run-pm.sh`
...
- **Single host only.** flock and mkdir-locks are both fs-local; on NFS the
  ...
  `flock(2)` man page on Linux). autopilot-swarm targets a single
...
- **No fallback for clustered / NFS runners.** Single-host assumption is
  durable for autopilot-swarm v1; revisit when a multi-host deployment
