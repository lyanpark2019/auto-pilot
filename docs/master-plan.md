...
- ~~NOT yet proven live~~ **PROVEN 2026-06-10 (2nd live run)**: iter-1 bundle carried `project-context.md` (2184 lines) with `snapshot_shas.project_context` pinned; phase 1 merged `75b995d` after dual APPROVE; phase-2 worker commit recovered via cherry-pick after run stop.
- Live run 2 also surfaced **F-6 (P1)**: headless PM dispatched reviewers in background then exited — one-shot `-p` session has no wake-up, loop stalls re-validating (3 iters burned). Fixed: `prompts/headless.md` + `iteration.md` now mandate synchronous dispatch / no in-flight exit. Resume logic itself proven sound (iter-3 collected orphan reviews and merged).
