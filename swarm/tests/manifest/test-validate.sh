...
# REPO_ROOT = the swarm/ subtree; the merged auto-pilot plugin manifest lives
# one level up at <plugin-root>/.claude-plugin/plugin.json.
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
VALIDATOR="${REPO_ROOT}/scripts/check-manifest.sh"
REAL_MANIFEST="$(cd "${REPO_ROOT}/.." && pwd)/.claude-plugin/plugin.json"
