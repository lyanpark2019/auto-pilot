...
8e1d030 docs(quality): refresh v087 final rescore prompt
- Metrics from scripts/quality/metric_snapshot.py:
  ...
  - final v0.8.7: long_functions_gt40=15, broad_exceptions=8, print_calls=50, scripts coverage 87.34%, root pytest 608 passed, vault pytest 97 passed, strict mypy source files=46, strict vault mypy pilots=11.
- Local verification at HEAD:
  - python3 -m pytest tests/ -q => 608 passed.
  - (cd vault && python3 -m pytest tests/ -q) => 97 passed.
  - python3 -m pytest tests/ -q --cov=scripts --cov-fail-under=80 => 608 passed, scripts coverage 87.34%.
  - python3 -m mypy => Success: no issues found in 46 source files.
...
- Major changes:
  ...
  - mypy.ini strict scope expanded from 2 vault pilots to 11 vault files; tests/test_mypy_scope.py guards scope.
  - Broad exception cleanup lowered broad handlers 30 -> 8 across lockfile, scan_code, codex guard, graphify loop, dashboard/cost/scoring, migration worker, asset registry, MCP audit status, selftest, and restructure paths.
  - Raw print cleanup lowered print calls 168 -> 50 across vault pipeline/scripts, orchestrator, eval CLI, asset registry, MCP, dispatch, drift, and phase06.
  - Long function cleanup lowered >40-line functions 34 -> 15 across hooks, restructure loop/phases, NotebookLM classifier/materializer, migration worker, dashboard, asset registry, eval runner/CLI, fix planner, and score_content.
  - scripts/graphify_vault_loop.py default_runner now has a 120s subprocess timeout with tests.
  - tests/test_asset_registry_check.py added; scripts coverage moved 80.06% -> 87.34%.
