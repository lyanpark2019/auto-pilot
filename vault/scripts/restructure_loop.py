...
def make_ctx(args) -> PhaseContext:
...
def run_one(phase_cls, ctx: PhaseContext, state: dict, force: bool = False) -> bool:
    """Returns True if phase completed (or was already done)."""
    phase = phase_cls(ctx)
    p_state = state["phases"].setdefault(phase.name, {"status": "pending"})
    if p_state.get("status") == "completed" and not force:
        ctx.trace(f"--- {phase.name}: already completed (skip) ---")
        return True
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            result = PhaseResult(status="failed", detail=f"exception: {e!r}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.reset and ctx.state_path.exists():
    ...
    # Final report
    summary = {
        pc.name: state["phases"].get(pc.name, {}).get("status", "unknown")
        for pc in PHASES
    }
    ctx.trace("\n=== Final phase summary ===")
    ...
    lines.append(f"- Run mode: {'dry-run' if args.dry_run else 'live'}")
