...
- **`class` (optional, controlled vocab):** tag each finding with the single closest defect class from this fixed vocabulary — `index-out-of-bounds`, `null-deref`, `unguarded-empty-input`, `off-by-one`, `unchecked-return`, `resource-leak`, `race-condition`, `injection`, `missing-input-validation`, `incorrect-error-handling`, `type-confusion`, `scope-violation`, `missing-test`, `spec-noncompliance`, `doc-drift`, `dead-code`. **OMIT the field when none fits** — never force a class. **Tie-break (pick the MOST SPECIFIC, so both reviewers converge on the same tag for one defect):** a None/null deref caused by an empty collection/string → `unguarded-empty-input` (not `null-deref`); a `len-1` / `p==1.0` / boundary index error → `index-out-of-bounds` (not `off-by-one`); an unchecked or ignored return/error value → `unchecked-return` (reserve `incorrect-error-handling` for a handler that DOES run but is wrong/partial). The learning miner keys recurrence on this canonical tag (not your free `issue` wording), so a recurring defect only accumulates toward promotion when BOTH reviewers classify it consistently. An out-of-vocab or omitted class is always safe — the miner falls back to the issue text and the schema never rejects it.
